...
//            NSString *serverAddress = [HTServerConfig getHTServerAddr];
            url = [DATA_ADDRESS stringByAppendingString:url];
            NSLog(@"数据请求---%@",url);
        }
...
//        NSData *imageData = UIImageJPEGRepresentation(image, 1);
        NSData * imageData = [Command reSizeImageData:image maxImageSize:800 maxSizeWithKB:2048];
...
+ (void)dismissSuccessHUD
{
    dispatch_main_async_safe(^{
       ...
       // [HTShowMessageView dismissSuccessView:@""];
...
+ (void)dismissErrorHUD
{
    dispatch_main_async_safe(^{
       ...
       // [HTShowMessageView dismissErrorView:@""];
        [SVProgressHUD dismiss];
    });
