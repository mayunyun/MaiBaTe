...
@interface WLMapViewController ()<MAMapViewDelegate, AMapLocationManagerDelegate,AMapSearchDelegate,UISearchBarDelegate,UITableViewDelegate,UITableViewDataSource>
{
    ...
    CusAnnotationView *annotationView;
...
- (void)initMapView
{
    if (self.mapView == nil)
    {
        self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 64, UIScreenW, UIScreenH-64-55*MYWIDTH)];
...
//            [self.search AMapDistrictSearch:dist];
    ...
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake((self.mapView.width - 25) / 2, self.mapView.height/2-45, 25, 45)];
    
    if ([self.type isEqualToString:@"1"]) {
        imageView.image = [UIImage imageNamed:@"终点"];
    }else{
        imageView.image = [UIImage imageNamed:@"起点"];
    }
    ...
    _mapLab.textColor = UIColorFromRGB(0x333333);
...
- (UIView *)bgView{
    if (_bgView == nil) {
        _bgView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, UIScreenW, UIScreenH-64)];
        _bgView.backgroundColor = [UIColor clearColor];
        [self.view addSubview:_bgView];
        
        _tableview = [[UITableView alloc]initWithFrame:CGRectMake(60*MYWIDTH, 0, 260, UIScreenH/2)];
        _tableview.layer.cornerRadius = 1.0;
        _tableview.layer.borderColor = UIColorFromRGB(0x555555).CGColor;
        ...
        UIButton *but = [[UIButton alloc]initWithFrame:CGRectMake(0, _tableview.bottom, UIScreenW, UIScreenH/2)];
