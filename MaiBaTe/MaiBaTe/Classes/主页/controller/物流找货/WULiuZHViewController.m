...
@interface WULiuZHViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation WULiuZHViewController
{
    NSInteger _page;
...
- (void)loadData
{
    ...
    //
    NSString *URLStr = @"/mbtwz/logisticsgoods?action=selectLogisticsOrderList";
    ...
    [HTNetWorking postWithUrl:URLStr refreshCache:YES params:params success:^(id response) {
        ...
        if ([[diction objectForKey:@"flag"] intValue]==200) {
            if ([[[diction objectForKey:@"response"] objectForKey:@"rows"] count]) {
                for (NSDictionary *dic in [[diction objectForKey:@"response"] objectForKey:@"rows"]) {
                    //建立模型
