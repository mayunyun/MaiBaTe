...
- (void)viewDidLoad {
    ...
    [HTNetWorking postWithUrl:URLStr refreshCache:YES params:params success:^(id response) {
        arr = [NSJSONSerialization JSONObjectWithData:response options:NSJSONReadingMutableContainers error:nil];
        NSSLog(@"城市地址%@",arr);
        if (arr.count) {
...
- (void)creatUI{
    UIScrollView *_bgsView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 64, UIScreenW, _typeviewheight+180*MYWIDTH)];
    ...
    yuan.backgroundColor = UIColorFromRGB(MYColor);
    ...
    [button setBackgroundImage:[UIImage imageNamed:@"矩形1"] forState:UIControlStateNormal];
