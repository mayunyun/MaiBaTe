...
-(instancetype)initWithFrame:(CGRect)frame{
    if (self =[super initWithFrame:frame]) {
        self.backgroundColor = UIColorFromRGB(MYBack);
...
-(void)dataHeadView:(NSMutableArray *)data{
    UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(0, 12*MYWIDTH, UIScreenW, 186*MYWIDTH)];
    ...
    UIView *xian = [[UIView alloc]initWithFrame:CGRectMake(15*MYWIDTH, 30*MYWIDTH, UIScreenW-30*MYWIDTH, 0.5)];
    ...
    for (CarBrandModel *model in data) {
        ...
        NSString *strurl = [NSString stringWithFormat:@"%@/%@%@",PHOTO_ADDRESS,model.folder,model.autoname];
    ...
    for (int i = 0; i<[_titleArr count]; i++) {
        ...
        CGFloat appviewx = (UIScreenW/_totalloc)/2 - appvieww/2 + (UIScreenW/_totalloc) * (i%_totalloc);
        ...
        //
        UIButton *but = [[UIButton alloc]initWithFrame:CGRectMake(appviewx+appvieww/2-(UIScreenW/_totalloc)/2, appviewy+appviewh, UIScreenW/_totalloc, 25)];
        but.tag = i;
    ...
    if (data.count >5) {
        bgview.frame = CGRectMake(0, 12*MYWIDTH, UIScreenW, 186*MYWIDTH);
    }else if (data.count>0){
        bgview.frame = CGRectMake(0, 12*MYWIDTH, UIScreenW, 78*MYWIDTH);
    }else{
