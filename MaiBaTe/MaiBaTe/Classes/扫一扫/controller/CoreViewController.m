        ...
        if ([str rangeOfString:@"true"].location!=NSNotFound) {
            ...
            [HTNetWorking postWithUrl:URLStr refreshCache:YES params:parmas success:^(id response) {
                ...
                if ([str rangeOfString:@"true"].location!=NSNotFound) {
                    ...
                    [user setObject:data forKey:ChongDianNumer];
                    [user synchronize];
                    NSLog(@"%@",data);
...
- (void)setOverlayPickerView
{
    ...
    UIButton *zhongDuanbut = [[UIButton alloc]initWithFrame:CGRectMake(UIScreenW/2, 0, UIScreenW/2, blackBG.height)];
    [zhongDuanbut addTarget:self action:@selector(zhongDuanClick) forControlEvents:UIControlEventTouchUpInside];
    [blackBG addSubview:zhongDuanbut];
}
- (void)zhongDuanClick{    
    ZhongDuanViewController *zdvc = [[ZhongDuanViewController alloc]init];
    [self presentViewController:zdvc animated:YES completion:nil];
}
//
