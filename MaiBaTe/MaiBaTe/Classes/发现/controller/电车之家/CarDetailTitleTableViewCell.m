...
- (void)creatUI{
    ...
    _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(15*GMYWIDTH, 25*GMYWIDTH, UIScreenW - 30*GMYWIDTH, 20*GMYWIDTH)];
    [self addSubview:_titleLabel];
    _titleLabel.backgroundColor = [UIColor clearColor];
    _titleLabel.font = [UIFont fontWithName:@"PingFangSC-Regular" size:20.0];
    
    _locopriceLabel = [[UILabel alloc]initWithFrame:CGRectMake(_titleLabel.left, _titleLabel.bottom+15*GMYWIDTH, UIScreenW - 30*GMYWIDTH, 15*GMYWIDTH)];
...
- (void)setData
{
    ...
    if (!IsEmptyValue(model.localprice)) {
        _locopriceLabel.text = [NSString stringWithFormat:@"本地参考报价：%@",money];
    ...
    if (!IsEmptyValue(model.factoryprice)) {
        _msrpLabel.text = [NSString stringWithFormat:@"厂商指导价：%@",msrmoeystr];
    }else{
        _msrpLabel.text = @"";
