...
- (UITableView *)tableview{
    if (_tableview == nil) {
        ...
        _textfield = [[UITextField alloc]initWithFrame:CGRectMake(15, 0, UIScreenW-75, 43.5*MYWIDTH)];//- 50
        ...
        UIButton *chaxun = [[UIButton alloc]initWithFrame:CGRectMake(UIScreenW-15-43.5*MYWIDTH, 0, 43.5*MYWIDTH, 43.5*MYWIDTH)];
...
- (void)loadNew{
    ...
    [HTNetWorking postWithUrl:URLStr refreshCache:YES params:params success:^(id response) {
        ...
        NSSLog(@"系列列表%@",responseJSON);
        
        if (!IsEmptyValue(responseJSON)) {
            for (NSDictionary* dict in [responseJSON objectForKey:@"rows"]) {
