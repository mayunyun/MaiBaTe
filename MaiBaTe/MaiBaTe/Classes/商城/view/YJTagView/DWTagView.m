    ...
    if ([collectionView isEqual:self.collectionView]) {
        for (int i=0; i<[self.dataSource DWnumOfItemstagView:self]; i++) {
            ...
            if (i==indexPath.row) {
                ...
                lab.backgroundColor = UIColorFromRGB(MYColor);
            }
        }
...
-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewFlowLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [TagCollectionViewCell getSizeWithContent:[self.dataSource DWtagView:self titleForItemAtIndex:indexPath.row] maxWidth:_collectionView.frame.size.width customHeight:self.cellHeight];
}
