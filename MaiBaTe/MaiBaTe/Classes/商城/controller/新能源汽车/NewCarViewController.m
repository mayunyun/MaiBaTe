...
@interface NewCarViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>{
    int _numer;
}
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"新能源汽车";
...
- (void)loadNewData{
    ...
    [HTNetWorking postWithUrl:URLStr refreshCache:YES params:nil success:^(id response) {
        ...
        for (NSDictionary *dicId in Arr) {
            if ([[dicId objectForKey:@"id"] isEqualToString:@"537"]||[[dicId objectForKey:@"id"] isEqualToString:@"538"]) {
...
- (void)AllProductData{
    ...
    NSDictionary* params = @{@"params":[NSString stringWithFormat:@"{\"id\":\"%@\"}",_idArr[_numer]]};
    NSLog(@"%@",params);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    YuYueCarViewController *yuyuecar = [[YuYueCarViewController alloc]init];
    DetailsViewController *detailsVc = [[DetailsViewController alloc]init];
    if (_idArr.count) {
        for (int i = 0; i < _idArr.count; i++)
        {
            if (indexPath.section==i) {
                if ([_idArr[i] isEqualToString:@"537"]||[_idArr[i] isEqualToString:@"538"]){
                    if (self.dataArr.count) {
                        ShopAddrModel *model = self.dataArr[indexPath.section][indexPath.row];
                        yuyuecar.id = model.id;
                    }
                    [self.navigationController pushViewController:yuyuecar animated:YES];
                }else{
                    if (self.dataArr.count) {
                        ShopAddrModel *model = self.dataArr[indexPath.section][indexPath.row];
