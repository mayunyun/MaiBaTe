...
//    [view addSubview:pay];
    ...
    UILabel *pay1 = [[UILabel alloc]initWithFrame:CGRectMake(UIScreenW-90, 0, 80, 50*MYWIDTH)];
    pay1.text = [NSString stringWithFormat:@"￥%.2f",[_Ordermodel.totalmoney floatValue]];
    ...
    pay1.textColor = UIColorFromRGB(0xFFB400);
...
- (void)dataRequest{
    ...
    NSLog(@"%@",params);
    [HTNetWorking postWithUrl:@"/mbtwz/scshop?action=getOneOrder" refreshCache:YES params:params success:^(id response) {
...
- (void)butwhihGo:(UIButton *)but{
    ...
    NSLog(@"%@",params);
    [HTNetWorking postWithUrl:@"/mbtwz/scshop?action=updateOrder_4" refreshCache:YES params:params success:^(id response) {
