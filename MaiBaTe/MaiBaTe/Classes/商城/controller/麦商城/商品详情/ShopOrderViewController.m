...
- (UITableView *)TableView{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, UIScreenW, UIScreenH-45-64) style:UITableViewStyleGrouped];
        ...
        food.backgroundColor = UIColorFromRGB(MYBack);
...
- (void)fooderUIView{
    ...
    //
    UIView *viewal = [[UIView alloc]initWithFrame:CGRectMake(0, UIScreenH-45-64, UIScreenW, 45)];
    if (statusbarHeight>20) {
        viewal.frame = CGRectMake(0, UIScreenH-45-64-34, UIScreenW, 45);
    }
    ...
    _monlab.text = [NSString stringWithFormat:@"￥%@",@"0.00"];
    ...
    [monBut setBackgroundColor:UIColorFromRGB(MYColor)];
...
- (void)setSMAlertWithView{
    ...
    UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(30*MYWIDTH, 0, UIScreenW-60*MYWIDTH, 240*MYWIDTH)];
    ...
    UILabel *yueLab = [[UILabel alloc]initWithFrame:CGRectMake(bgview.width/2-10, bgview.bottom-30*MYWIDTH, 200, 30*MYWIDTH)];
    yueLab.text = _YuENumer;
...
- (void)zhifuButClicked:(UIButton *)button{
    ...
    if (button.tag == 550) {//支付宝
        jxt_showAlertTwoButton(@"提示", @"您确定使用支付宝支付吗", @"取消", ^(NSInteger buttonIndex) {
            ...
            if ([_youhuimodel.cheap_equal_money floatValue]==0) {
                [Paydetail zhiFuBaoname:@"测试" titile:@"测试" price:[_monlab.text substringFromIndex:1] orderId:[NSString stringWithFormat:@"%@-%@-0",_orderId,[user objectForKey:USERID]] notice:@"0"];
            }else{
                [Paydetail zhiFuBaoname:@"测试" titile:@"测试" price:[_monlab.text substringFromIndex:1] orderId:[NSString stringWithFormat:@"%@-%@-%@",_orderId,[user objectForKey:USERID],_youhuimodel.couponid] notice:@"0"];
            }
        });
        ...
        jxt_showAlertTwoButton(@"提示", @"您确定使用微信支付吗", @"取消", ^(NSInteger buttonIndex) {
            ...
            if ([_youhuimodel.cheap_equal_money floatValue]==0) {
                [Paydetail wxname:@"麦巴特充值" titile:@"麦巴特充值" price:[_monlab.text substringFromIndex:1] orderId:[NSString stringWithFormat:@"%@-%@-0",_orderId,[user objectForKey:USERID]] notice:@"0"];
            }else{
                [Paydetail wxname:@"麦巴特充值" titile:@"麦巴特充值" price:[_monlab.text substringFromIndex:1] orderId:[NSString stringWithFormat:@"%@-%@-%@",_orderId,[user objectForKey:USERID],_youhuimodel.couponid] notice:@"0"];
            }
        });
        
        
    }else if (button.tag == 552){//余额支付
        if ([[_monlab.text substringFromIndex:1] floatValue]>[_YuENumer floatValue]) {
            ...
            jxt_showAlertTwoButton(@"提示", @"您确定使用余额支付吗", @"取消", ^(NSInteger buttonIndex) {
                ...
                NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"orderno\":\"%@\",\"totalmoney\":\"%.2f\",\"conpouid\":\"%@\"}",_orderId,[[_monlab.text substringFromIndex:1] floatValue]+[_youhuimodel.cheap_equal_money floatValue],[Command convertNull:_youhuimodel.couponid]]};
                
                NSLog(@"%@",params);
                [HTNetWorking postWithUrl:@"/mbtwz/scshop?action=orderYuEZhiFu" refreshCache:YES params:params success:^(id response) {
    ...
    NSLog(@"%@",params);
    [HTNetWorking postWithUrl:@"/mbtwz/address?action=updateorderaddress" refreshCache:YES params:params success:^(id response) {
        NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        NSLog(@"更新订单地址%@",str);
...
- (void)JiFenzhifuButClicked{
    ...
    if ([[_monlab.text substringToIndex:2] floatValue]>[_jifen floatValue]) {
        ...
        jxt_showAlertTwoButton(@"提示", @"您确定使用积分支付吗", @"取消", ^(NSInteger buttonIndex) {
            ...
            NSDictionary* data = @{@"data":[NSString stringWithFormat:@"{\"orderno\":\"%@\",\"totalmoney\":\"%@\"}",_orderId,[_monlab.text substringToIndex:_monlab.text.length-2]]};
            NSLog(@"%@",data);
            [HTNetWorking postWithUrl:@"/mbtwz/scshop?action=orderScoresZhiFu" refreshCache:YES params:data success:^(id response) {
                NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
                NSLog(@"积分支付状态%@",str);
                if ([str rangeOfString:@"false"].location!=NSNotFound) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            UIImageView *image = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"chargeback"]];
        ...
        imageview.frame = CGRectMake(UIScreenW-20, 15*MYWIDTH, 10, 20*MYWIDTH);
        ...
        cell.contentView.backgroundColor = UIColorFromRGB(MYBack);
        ...
        if (indexPath.row == 0) {
            ...
            if (!IsEmptyValue(_dataArray)) {
                ...
                _monlab.text = [NSString stringWithFormat:@"￥%.2f",money];
                if ([_youhuimodel.cheap_equal_money floatValue]!=0) {
                    _monlab.text = [NSString stringWithFormat:@"￥%.2f",money-[_youhuimodel.cheap_equal_money floatValue]];
                }
            ...
            if (!IsEmptyValue(_dataArray)) {
                ...
                [cell.imgview sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@/%@/%@",PHOTO_ADDRESS,model.folder,model.autoname]] placeholderImage:[UIImage imageNamed:@"default_img_cell"]];
                ...
                cell.priceLab.text = [NSString stringWithFormat:@"￥%.2f",[model.price floatValue]];
                if ([_type intValue]==2) {
...
- (void)dataRequest{
    ...
    NSLog(@"%@",params);
    [HTNetWorking postWithUrl:@"/mbtwz/scshop?action=getProductOrder_0" refreshCache:YES params:params success:^(id response) {
...
- (void)getCustAddr{
    ...
    [HTNetWorking postWithUrl:@"/mbtwz/address?action=searchDefaultAddress" refreshCache:YES params:nil success:^(id response) {
        [SVProgressHUD dismiss];
        NSArray *array = [NSJSONSerialization JSONObjectWithData:response options:NSJSONReadingMutableContainers error:nil];
        NSLog(@"%@",array);
        if (!IsEmptyValue(array)) {
