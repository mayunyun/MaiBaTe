...
- (void)loadNewData{
    ...
    self.cycleScrollView.currentPageDotColor = UIColorFromRGB(MYColor); // 自定义分页控件小圆标颜色
...
- (void)threadRun{
    ...
    "}"
    "</style>";
    [webView loadHTMLString:[NSString stringWithFormat:@"%@%@",_model.note,linkCss] baseURL:[NSURL URLWithString:WEB_ADDRESS]];
...
-(void)createUI{
    ...
    xianview.backgroundColor = UIColorFromRGB(MYBack);
    ...
    _priceLab.textColor = UIColorFromRGB(0xFFB400);
    _priceLab.text = [NSString stringWithFormat:@"￥%.2f",[_model.price floatValue]];
    ...
    _colorview = [[YJTagView alloc] initWithFrame:CGRectMake(70+49*MYWIDTH, titleLabel1.y, UIScreenW-85-49*MYWIDTH, _colorviewheight)];
    _colorview.themeColor = UIColorFromRGB(MYColor);
    ...
    _typeview = [[YJTagView alloc] initWithFrame:CGRectMake(70+49*MYWIDTH, titleLabel2.y, UIScreenW-85-49*MYWIDTH, _typeviewheight)];
    _typeview.themeColor = UIColorFromRGB(MYColor);
    ...
    _shopcartCountView.frame = CGRectMake(70+49*MYWIDTH, titleLabel3.y, 100*MYWIDTH, 30*MYWIDTH);
    ...
    [shopCarBut setBackgroundColor:UIColorFromRGB(0xFFB400)];
    ...
    [shopCarBut setTitleColor:UIColorFromRGB(0x000000) forState:UIControlStateNormal];
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    webView.frame = CGRectMake(0, proDetailLabel.bottom, UIScreenW, documentHeight+20);
    bgview.frame = CGRectMake(0, UIScreenW/5*4, UIScreenW, proDetailLabel.bottom+webView.frame.size.height+20);
...
-(void)shopCarButClicked{
    [Command isloginRequest:^(bool str) {
        if (str) {
            ...
            NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"proid\":\"%@\",\"proname\":\"%@\",\"color\":\"%@\",\"fitcar\":\"%@\",\"count\":\"%@\",\"type\":\"%@\"}",_id,_model.proname,_colorNum,_typeNum,_countNum,_type]};
            NSLog(@"%@",params);
            [HTNetWorking postWithUrl:URLStr refreshCache:YES params:params success:^(id response) {
...
- (void)shoporderWithToUUIDJiFen{
    ...
    [SVProgressHUD showWithStatus:@"正在形成订单..."];
    NSString *URLStr = @"/mbtwz/scshop?action=insertProOrder_0";
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"id\":\"%@\",\"proname\":\"%@\",\"color\":\"%@\",\"specification\":\"%@\",\"totalcount\":\"%@\",\"type\":\"%@\"}",_id,_model.proname,_colorNum,_typeNum,_countNum,_type]};
    NSLog(@"%@",params);
    [HTNetWorking postWithUrl:URLStr refreshCache:YES params:params success:^(id response) {
        [SVProgressHUD dismiss];
