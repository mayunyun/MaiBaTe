...
- (void)setBookModel:(SABookModel *)bookModel{
    ...
    NSString *image = [NSString stringWithFormat:@"%@/%@%@",PHOTO_ADDRESS,bookModel.folder,bookModel.autoname];
    NSLog(@"%@",image);
    ...
    if ([bookModel.type integerValue]==2) {
        ...
        self.price.text = [NSString stringWithFormat:@"%.2f元",[bookModel.price floatValue]];
        [self changeTextColor:self.price Txt:self.price.text changeTxt:[NSString stringWithFormat:@"%.2f",[bookModel.price floatValue]]];
    }
