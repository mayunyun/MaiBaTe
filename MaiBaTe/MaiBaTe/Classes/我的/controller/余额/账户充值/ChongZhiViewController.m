...
- (void)setChongzhiUIView{
    UILabel *textLab = [[UILabel alloc]initWithFrame:CGRectMake(15*MYWIDTH, 64+Heihht, 60, 20*MYWIDTH)];
    if (statusbarHeight>20) {
        textLab.frame = CGRectMake(15*MYWIDTH, 88+Heihht, 60, 20*MYWIDTH);
    }
    textLab.text = @"充值金额";
    ...
    for (int i=0; i<_arr.count; i++) {
        ...
        [numerbtn setTitleColor:UIColorFromRGB(MYColor) forState:UIControlStateSelected];
    ...
    UILabel *yuanLab = [[UILabel alloc]initWithFrame:CGRectMake(_textField.right+10, textLab1.y, 30, 20*MYWIDTH)];
    yuanLab.text = @"元";
    yuanLab.textColor = UIColorFromRGB(0x333333);
    yuanLab.font = [UIFont systemFontOfSize:14];
    [self.view addSubview:yuanLab];
    
    UILabel *textLab2 = [[UILabel alloc]initWithFrame:CGRectMake(15*MYWIDTH, _textField.bottom+Heihht*1.5, 60, 20*MYWIDTH)];
    textLab2.text = @"充值方式";
    textLab2.textColor = UIColorFromRGB(0x333333);
    textLab2.font = [UIFont systemFontOfSize:14];
    [self.view addSubview:textLab2];

    
    //
    UIView *xian = [[UIView alloc]initWithFrame:CGRectMake(15*MYWIDTH, textLab2.bottom+Heihht*0.8, UIScreenW-30*MYWIDTH, 1)];
    xian.backgroundColor = UIColorFromRGB(0xDDDDDD);
    [self.view addSubview:xian];
    
    UIImageView *zhiImage = [[UIImageView alloc]initWithFrame:CGRectMake(15*MYWIDTH, xian.bottom+Heihht, 30*MYWIDTH, 30*MYWIDTH)];
    zhiImage.image = [UIImage imageNamed:@"支付宝"];
    [self.view addSubview:zhiImage];
    
    UILabel *textLab3 = [[UILabel alloc]initWithFrame:CGRectMake(15*MYWIDTH+zhiImage.right, zhiImage.y, 80, 30*MYWIDTH)];
    textLab3.text = @"支付宝支付";
    ...
    [chooseBut setFrame:CGRectMake(UIScreenW-50*MYWIDTH,zhiImage.y, 30*MYWIDTH, 30*MYWIDTH)];
    ;
    chooseBut.tag = 210;
    chooseBut.selected = YES;
    [chooseBut setImage:[UIImage imageNamed:@"未选灰"] forState:UIControlStateNormal];
    [chooseBut setImage:[UIImage imageNamed:@"选中亮"] forState:UIControlStateSelected];
    ...
    UIImageView *weiImage = [[UIImageView alloc]initWithFrame:CGRectMake(15*MYWIDTH, zhiImage.bottom+Heihht+3*MYWIDTH, 30*MYWIDTH, 25*MYWIDTH)];
    weiImage.image = [UIImage imageNamed:@"微信"];
    [self.view addSubview:weiImage];
    
    UILabel *textLab4 = [[UILabel alloc]initWithFrame:CGRectMake(15*MYWIDTH+weiImage.right, weiImage.y, 80, 30*MYWIDTH)];
    textLab4.text = @"微信支付";
    ...
    [chooseBut1 setFrame:CGRectMake(UIScreenW-50*MYWIDTH,weiImage.y, 30*MYWIDTH, 30*MYWIDTH)];
    ;
    chooseBut1.tag = 211;
    [chooseBut1 setImage:[UIImage imageNamed:@"未选灰"] forState:UIControlStateNormal];
    [chooseBut1 setImage:[UIImage imageNamed:@"选中亮"] forState:UIControlStateSelected];
    ...
    [_zhifuBut setFrame:CGRectMake(15*MYWIDTH,weiImage.bottom+Heihht*2.5, UIScreenW-30*MYWIDTH, 50*MYWIDTH)];
    ;
    [_zhifuBut setBackgroundImage:[UIImage imageNamed:@"矩形1"] forState:UIControlStateNormal];
...
- (void)loadNew
{
    ...
    [HTNetWorking postWithUrl:[NSString stringWithFormat:@"%@%@",DATA_ADDRESS,URL] refreshCache:YES params:nil success:^(id response) {
        ...
        if (zbutton.selected == YES) {
            ...
            [Paydetail wxname:@"麦巴特充值" titile:@"麦巴特充值" price:_payMoney orderId:_orderId notice:@"1"];
        }
        NSLog(@">>>%@   %@",str0,str2);
