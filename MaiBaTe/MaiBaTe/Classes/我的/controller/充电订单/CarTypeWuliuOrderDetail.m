...
@interface CarTypeWuliuOrderDetail ()<UITableViewDataSource,UITableViewDelegate,CDZStarsControlDelegate,UITextViewDelegate>
...
@implementation CarTypeWuliuOrderDetail
{
    UIView * starView;
...
-(void)requestDetail{
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=searchorder"];
...
-(void)requestDataForRate{
    ...
    [HTNetWorking postWithUrl:url refreshCache:YES params:nil success:^(id response) {
        ...
        if (arr.count == 0||arr == nil) {
            ...
            fee = [NSString stringWithFormat:@"%@",arr[0][@"fee"]];
        }
...
-(void)CommitBtnClicked:(UIButton *)sender{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=evlateOrder"];
    NSDictionary * dic;
    if ([_inputTV.text isEqualToString:@""]) {//默认不填任何信息是好评
        dic =@{@"id":self.model.id,@"driver_fraction":starScore,@"note":@"默认好评",@"driverid":self.model.driver_custid};
    }else{
        dic =@{@"id":self.model.id,@"driver_fraction":starScore,@"note":_inputTV.text,@"driverid":self.model.driver_custid};
    }
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [HTNetWorking postWithUrl:url refreshCache:YES params:KCparams success:^(id response) {
        [SVProgressHUD dismiss];
...
-(void)startOrder{
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=startOrder"];
    NSDictionary * dic =@{@"orderno":self.model.owner_orderno};
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [HTNetWorking postWithUrl:url refreshCache:YES params:KCparams success:^(id response) {
        [SVProgressHUD dismiss];
...
-(void)endingOrderData{
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=finishOrder"];
    WuliuOrderModel * model = _detailArr[0];
    NSDictionary * dic =@{@"orderno":self.model.owner_orderno,
                          @"order_money":model.owner_totalprice,
                          @"owner_createtime":model.owner_createtime,
                          @"siji_findtime":model.siji_findtime,
...
-(void)DriverSyData{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=overTime"];
...
-(void)cancelOrderData{
    NSString *url = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/mbtwz/logisticssendwz?action=cancelOrder"];
    NSDictionary * dic =@{@"orderno":self.model.owner_orderno,@"owner_createtime":self.model.owner_createtime,@"siji_findtime":self.model.siji_findtime,@"order_money":self.model.owner_totalprice};
    NSDictionary* KCparams = @{@"data":[Command dictionaryToJson:dic]};//
    [HTNetWorking postWithUrl:url refreshCache:YES params:KCparams success:^(id response) {
        [SVProgressHUD dismiss];
        NSString * str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        ...
        }else if ([str isEqualToString:@"\"true1\""]){
            feeLabel.hidden = YES;
            [SMAlert showCustomView:canView];
        }else{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        cell.price.text = [NSString stringWithFormat:@"%.2f元",[self.model.owner_totalprice floatValue]];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    }else{
        //未接单
        ...
        if ([[Command convertNull:driver_custId] isEqualToString:@""]||[model.driver_custid intValue] == 0) {
            ...
            if (_detailArr.count) {
                ...
                cell.driverPhone.text = [NSString stringWithFormat:@"%@",[NSString stringWithFormat:@"%@",model.driver_phone]];
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            return cell;
        }
