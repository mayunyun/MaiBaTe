...
@interface CarTapeAllWuliuOrderListVC ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation CarTapeAllWuliuOrderListVC
...
- (void)backToLastViewController:(UIButton *)button{
    if ([self.isBack isEqualToString:@"2"]) {
        ...
        for (UIViewController * controller in self.navigationController.viewControllers) {
            ...
            }else if ([controller isKindOfClass:[WuLiuFHViewController class]]) {
                [self.navigationController popToViewController:controller animated:YES];
            }
        }
    }else{
