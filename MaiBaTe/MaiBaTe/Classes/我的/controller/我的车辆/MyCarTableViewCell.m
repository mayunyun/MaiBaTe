...
- (void)setData:(MyCarModel *)data
{
    ...
    .leftSpaceToView(_bgview, margin)
    .topSpaceToView(_iconView1, 2*margin)
    .widthIs(margin)
    .heightIs(margin);
    
    _iconView3.sd_layout
    .leftSpaceToView(_bgview, margin)
    .topSpaceToView(_iconView2, 2*margin)
    .widthIs(margin)
    .heightIs(margin);
...
//    .heightIs(margin);
    ...
    .leftSpaceToView(_bgview, UIScreenW/2-15*MYWIDTH + margin)
    .topSpaceToView(_iconView4, 2*margin)
    .widthIs(margin)
    .heightIs(margin);
...
//    .heightIs(20*MYWIDTH);
    ...
    _titleView.text = [NSString stringWithFormat:@"车牌号:%@",_data.cartype];
    _titleView1.text = [NSString stringWithFormat:@"车辆名称:%@",_data.maxpeople];
    _titleView2.text = [NSString stringWithFormat:@"车速:%@km/h",_data.chesu];
    _titleView3.text = [NSString stringWithFormat:@"总电流:%@A",_data.zongdianliu];
    _titleView4.text = [NSString stringWithFormat:@"总电压:%@V",_data.zongdianya];
    //_titleView5.text = [NSString stringWithFormat:@"电池容量:%@kwh",_data.engine];
    _titleView6.text = [NSString stringWithFormat:@"DC-DC温度:%@℃",_data.wendu];
    [self changeTextColor:_titleView1 Txt:_titleView1.text changeTxt:[NSString stringWithFormat:@"%@",_data.maxpeople]];
    [self changeTextColor:_titleView2 Txt:_titleView2.text changeTxt:[NSString stringWithFormat:@"%@",_data.chesu]];
    [self changeTextColor:_titleView3 Txt:_titleView3.text changeTxt:[NSString stringWithFormat:@"%@",_data.zongdianliu]];
    [self changeTextColor:_titleView4 Txt:_titleView4.text changeTxt:[NSString stringWithFormat:@"%@",_data.zongdianya]];
    [self changeTextColor:_titleView6 Txt:_titleView6.text changeTxt:[NSString stringWithFormat:@"%@",_data.wendu]];
...
- (UIImageView *)iconView2
{
    if(_iconView2 ==nil)
    {
        ...
        _iconView2.image = [UIImage imageNamed:@"速度"];
...
}- (UIImageView *)iconView3
{
    if(_iconView3 ==nil)
    {
        ...
        _iconView3.image = [UIImage imageNamed:@"电池"];
...
}- (UIImageView *)iconView4
{
    if(_iconView4 ==nil)
    {
        ...
        _iconView4.image = [UIImage imageNamed:@"行驶里程"];
...
- (UIImageView *)iconView6
{
    if(_iconView6 ==nil)
    {
        ...
        _iconView6.image = [UIImage imageNamed:@"功率"];
...
- (UIButton *)deleteBut
{
    if(_deleteBut ==nil)
    {
        ...
        [_deleteBut setTitleColor:UIColorFromRGB(0x666666) forState:UIControlStateNormal];
        _deleteBut.titleLabel.font = [UIFont systemFontOfSize:13];
        [_bgview addSubview: _deleteBut];
    }
    return _deleteBut;
}
- (void)awakeFromNib {
