...
@interface PayPwdViewController ()<UITextFieldDelegate>
{
...
- (void)creatUI{
    ...
    _uppopView.layer.cornerRadius = 10*GMYWIDTH;
    ...
    _phoneField.font = [UIFont systemFontOfSize:13*GMYWIDTH];
    ...
    _ecodeBtn.titleLabel.font = [UIFont systemFontOfSize:14*GMYWIDTH];
    ...
    _againpwdField.keyboardType = UIKeyboardTypeNumberPad;
    ...
    if ([_ecodeField.text isEqualToString:_smscode]&&!IsEmptyValue(_ecodeField.text)) {
        if ([Command validatePwd:_pwdField.text]) {
            if ([Command validatePwd:_pwdField.text]) {
                ...
                jxt_showToastTitle(@"密码格式为6位数字", 1);
            }
        }else{
            jxt_showToastTitle(@"密码格式为6位数字", 1);
        }
...
- (void)registRequest{
    /*
     /mbtwz/register?action=upzhifuPassword
