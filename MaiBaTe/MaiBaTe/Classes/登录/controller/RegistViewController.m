...
- (void)creatUI{
    ...
    _nextBtn.layer.shadowRadius= 10;
    _nextBtn.clipsToBounds = false;
    [self.view addSubview:_nextBtn];
    [_nextBtn addTarget:self action:@selector(nextBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    
    _downpopView = [[UIView alloc]initWithFrame:CGRectMake(36*GMYWIDTH, 0.294*UIScreenH, UIScreenW - 72*GMYWIDTH, 240*GMYWIDTH)];
    ...
    UIImageView* nameView = [[UIImageView alloc]initWithFrame:CGRectMake(15*GMYWIDTH, 35*GMYWIDTH, _downpopView.width - 30*GMYWIDTH, 42*GMYWIDTH)];
    nameView.userInteractionEnabled = YES;
    nameView.image = [UIImage imageNamed:@"nameView"];
    [_downpopView addSubview:nameView];
    UIImageView* nameimgView = [[UIImageView alloc]initWithFrame:CGRectMake(15*GMYWIDTH, 8*GMYWIDTH, 20*GMYWIDTH, nameView.height - 16*GMYWIDTH)];
    nameimgView.image = [UIImage imageNamed:@"nameimgView"];
    [nameView addSubview:nameimgView];
    _nameField = [[UITextField alloc]initWithFrame:CGRectMake(40*GMYWIDTH, 0, nameView.width - 40*GMYWIDTH, nameView.height)];
    _nameField.backgroundColor = [UIColor clearColor];
    _nameField.placeholder = @"请填写您的用户名";
    _nameField.textAlignment = NSTextAlignmentCenter;
    _nameField.font = [UIFont systemFontOfSize:13*GMYWIDTH];
    _nameField.delegate = self;
    [nameView addSubview:_nameField];
    [Command placeholderColor:_nameField str:_nameField.placeholder color:[UIColor whiteColor]];
    
    UIImageView* pwdView = [[UIImageView alloc]initWithFrame:CGRectMake(nameView.left, nameView.bottom+20*GMYWIDTH, nameView.width, nameView.height)];
...
- (void)okBtnClick:(UIButton*)sender{
    if (!IsEmptyValue(_nameField.text)) {
...
- (void)registRequest{
    ...
    NSDictionary* params = @{@"data":[NSString stringWithFormat:@"{\"custphone\":\"%@\",\"custname\":\"%@\",\"password\":\"%@\"}",_phoneField.text,_nameField.text,_pwdField.text]};
    [HTNetWorking postWithUrl:urlstr refreshCache:YES params:params success:^(id response) {
        NSString* str = [[NSString alloc]initWithData:response encoding:NSUTF8StringEncoding];
        NSSLog(@">>>%@",str);
